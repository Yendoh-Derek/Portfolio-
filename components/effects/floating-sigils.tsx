...
export function FloatingSigils() {
    ...
    useEffect(() => {
        ...
        // Define static nodes for the narrative patterns
        // 1. Neural Net Cluster
        ...
        function initNarrative() {
            ...
            circuitNodes.push({ x: trX - (isMobile ? 40 : 80), y: trY + 40, connectedTo: [3] });
            circuitNodes.push({ x: trX + (isMobile ? 30 : 60), y: trY + 60, connectedTo: [] });
            circuitNodes.push({ x: trX - (isMobile ? 40 : 80), y: trY + 120, connectedTo: [4] });
        ...
        function spawnPulse() {
            if (Math.random() < 0.5 && neuralNodes.length > 0) {
                ...
                const nextLayerNodes = neuralNodes.filter(n => n.layer === fromNode.layer + 1);
                
                if (nextLayerNodes.length > 0) {
                    const toNode = nextLayerNodes[Math.floor(Math.random() * nextLayerNodes.length)];
                    pulses.push({
                        x: fromNode.x,
                        y: fromNode.y,
                        startX: fromNode.x,
    ...
    return <canvas ref={canvasRef} className="fixed inset-0 overflow-hidden pointer-events-none z-0" />;
}
